...
# Load backend/.env at config import so ANY process that imports `core` resolves the
# correct DB/secrets — including standalone seed/audit scripts that import core before
# calling their own load_dotenv. override=False → never clobbers env already provided
# by the running app/supervisor. Prevents the "wrong-DB (aurora_fnb)" seed bug class.
try:
