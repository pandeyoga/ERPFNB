...
async def seed():
    ...
    # Find The Coffeeshop outlet + the latest validated DS
    alt = await db.outlets.find_one({"code": "CFS", "deleted_at": None})
    if alt:
        # Find baseline avg
        ...
        if not existing:
            ...
            print(f"  Injected anomalous sales for The Coffeeshop on {inject_date}: {spike:,.0f} (baseline {mean:,.0f})")
        else:
            print(f"  Sales entry for The Coffeeshop on {inject_date} already exists — skipping injection")
