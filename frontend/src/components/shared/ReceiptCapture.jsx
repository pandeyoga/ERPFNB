 ...
 *
 * Phase 8C update:
 *   - Auto-uploads the receipt to /api/uploads (category=receipt) on file pick.
 *     This persists the file (audit trail + future re-OCR without re-upload),
 *     gives us a `file_id`, and lets the server-side cache short-circuit
 *     duplicate OCR calls (SHA-256 hash).
 *   - Falls back to base64 path if upload fails (offline / size).
 *   - Returns the upload result via `onUploaded` so callers (e.g. Petty Cash,
 *     Urgent Purchase) can store `attachment_id` + `receipt_url`.
 *
 * Usage:
 *   <ReceiptCapture
 *     onExtracted={(data) => { ... }}
 *     onImage={(dataUrl) => { ... }}        // optional, for storing as receipt_url
 *     onUploaded={(att) => { ... }}          // optional, attachment metadata { id, url, ... }
 *     compact={false}                        // tighter layout when used in dialog
 *     sourceType="petty_cash"                // optional, attached to upload
 *     sourceId={pcId}                        // optional, attached to upload
 *     useUploadFlow={true}                   // default; set false to force base64-only
 *   />
 *
...
import { Camera, Upload, Sparkles, X, AlertTriangle, CheckCircle2, Zap } from "lucide-react";
...
export default function ReceiptCapture({
  ...
  const handleFile = async (file) => {
    ...
    // 1. Read as data URL for preview + base64 fallback
    ...
    reader.onload = async (e) => {
      ...
      // 2. Auto-upload to /api/uploads (preferred path for Phase 8C)
      if (useUploadFlow) {
        setUploading(true);
        try {
          ...
          const att = unwrap(res);
          if (att && att.id) {
            setFileId(att.id);
            if (onUploaded) onUploaded(att);
          }
        } catch (e) {
          // Non-fatal: we still have the base64
          logger.warn("Receipt upload failed; falling back to base64", { error: e.message });
        } finally {
          setUploading(false);
        }
      }
  ...
  const runOCR = async () => {
    ...
    setCacheHit(false);
    try {
      // Prefer file_id path (server-side cache + audit linkage)
      const body = fileId
        ? { file_id: fileId }
        : { image_base64: base64, mime_type: mime || "image/jpeg" };
      const res = await api.post("/ai/ocr/receipt", body);
      ...
      setExtracted(data);
      if (data._cache_hit) setCacheHit(true);
      if (onExtracted) onExtracted(data);
      toast.success(
        data._cache_hit
          ? "Struk sudah pernah diekstrak — pakai hasil cache (instan)"
          : "Struk berhasil di-extract",
      );
    } catch (e) {
  ...
  return (
    <div className={cn("space-y-3", compact && "text-sm")} data-testid="receipt-capture">
      {!imageData ? (
        ...
        <div className="glass-card-hover p-3 flex gap-3 items-start">
          ...
          <div className="flex-1 space-y-2">
            ...
            {!extracted && (
              <Button onClick={runOCR} disabled={extracting || uploading} className="rounded-full"
      ...
      {extracted && <ExtractedSummary data={extracted} cacheHit={cacheHit} />}
    </div>
  );
}
...
function UploadDropzone({ fileRef, onFile }) {
  ...
  return (
    <div
      className={cn(
        "glass-card-hover border-2 border-dashed border-foreground/15 p-5 text-center cursor-pointer transition-colors",
        dragOver && "border-aurora-3 bg-aurora-3/5",
      )}
...
function ExtractedSummary({ data, cacheHit }) {
