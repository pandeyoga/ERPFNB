...
export default function NumberSeries() {
  ...
  return (
    <div className="space-y-4" data-testid="number-series-page">
      <p className="text-sm text-muted-foreground inline-flex items-center gap-1.5">
        Nomor dokumen otomatis di-generate dari sini. Edit format dan padding via Master Data jika diperlukan.
        <InlineHelp id="number-series-format" size="xs" placement="right" />
      </p>
      {loading ? <LoadingState rows={6} /> : (
        items.length === 0 ? <EmptyState title="Belum ada series" /> : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3" data-testid="number-series-grid">
            {items.map((s) => (
              <div key={s.id} className="glass-card p-4" data-testid={`number-series-card-${s.code}`}>
